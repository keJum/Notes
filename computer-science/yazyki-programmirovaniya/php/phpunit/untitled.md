---
description: Позволяет тестировать классы в изоляции от внешних зависимостей.
---
...
* getMockFromWsdl\(\) — позволяет строить Mock объекты на основе описания из [WSDL](http://ru.wikipedia.org/wiki/WSDL);
...
Результат выполнения этого теста будет успешным, если при вызове метода process\(\) произойдет однократный вызов трех перечисленных методов: getTemperature\(\), getWord\(\), showWord\(\). Обратите внимание, что в тесте проверка вызова getWord\(\) стоит после проверки вызова showWord\(\), хотя в тестируемом методе наоборот. 

Для контроля порядка вызова методов в PHPUnit используется другая конструкция — at\(\). Поправим немного код нашего теста так чтобы PHPUnit проверил заодно очередность вызова методов:
...
Помимо упомянутых once\(\) и at\(\) для тестирования ожиданий вызовов в PHPUnit есть также следующие конструкции: any\(\), never\(\), atLeastOnce\(\) и exactly\($count\).

## Переопределение возвращаемого результата

Мы можем заставить интересующий нас метод вернуть любой результат какой захотим при помощи кострукции will\(\).
...
* static::staticMethod\(\) — из статических и динамических методов.
 ...
 Из-за [ограничений self](http://ru.php.net/manual/en/language.oop5.late-static-bindings.php#language.oop5.late-static-bindings.self) не будет работать подмена статических методов, вызываемых внутри класса таким способом:
